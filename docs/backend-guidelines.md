...
2. **変更の影響範囲の把握**
   ...
   - 新規追加 vs 既存修正 を区別
   - API仕様変更やDB変更の有無を確認
   - `git log` でコミット履歴を確認し、変更がどのように積まれたかを把握する
   - 途中で修正・リバートされた変更を認識し、最終的なコードの状態を正とする

**【2回目以降のレビューの場合】**

1. **前回レビュー以降の差分確認**
   - `git log` で前回レビュー以降に追加されたコミットを確認する
   - 前回の指摘に対する修正コミットの有無を把握する
   - 修正対応による副次的な影響範囲を確認
2. **前回指摘事項の確認（必ずファイルを再読み込みすること）**
   - 前回🔴修正必須として指摘したファイルを Read ツールで再読み込みし、現在のコード内容を確認する
   - 指摘が解消されたかの判断は、実際に読んだコードの状態に基づいて行う（前回レビュー結果の記述を鵜呑みにしない）
   - ユーザーが「対応不要」「意図的」等と判断した項目は、その判断を尊重する
   - 修正済みの指摘を繰り返さない
3. **レビュー継続判断**
