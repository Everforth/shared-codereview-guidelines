...
2. **変更の影響範囲の把握**
   ...
   - API仕様変更やDB変更の有無を確認
3. **変更ファイルの全文確認**
   - 変更されたファイルは変更量に関わらず全文を読み、設計の文脈を把握する
   - ただし指摘対象は変更箇所に限定し、差分外の既存コードへの指摘は行わない
...
変更ファイル一覧を確認し、各層に該当ファイルがあるか判定する。
該当ファイルがない層はスキップし、次の層へ進む。
スキップした層のガイドラインは読み込まない。
...
指摘を行う前に、以下の検証を必ず実施すること：

1. **パターンの実例確認**
   - 同種の既存コードを最低3件確認
   - ガイドラインの文言と実際のコードパターンを照合
   - 新規追加されたファイル・メソッドは既存パターンとの照合よりもガイドライン遵守を優先する

2. **判断の確信度による分類**
   - 既存コード3件以上で同一パターン確認 → 🔴修正必須として指摘可能
   - 既存コード1-2件または確信が持てない → 🟨確認として質問する
   - 既存コードで逆パターンが見つかった → 指摘しない

3. **確認せずに指摘することの禁止**
   - 既存コードを確認せずに「修正必須」を出すことを禁止
   - ガイドラインの字句のみに基づく指摘を禁止
...
該当層の変更ファイルは差分だけでなく必ず全文を読みながらレビューする（offset/limitを使わずファイル全体を読む）。
...
レビュー結果を出力する前に、該当層ガイドラインの必須チェック項目を1つずつ確認する。
この検証結果は最終出力に含めないが、検証は必ず実施すること。

```
（内部確認用）
- 項目名: ✅ 問題なし / ❌ 違反あり（具体的な問題）
- 項目名: ✅ 問題なし / ❌ 違反あり（具体的な問題）
...
```

違反が見つかった項目は🔴修正必須として出力する。
...
```
## レビュー状況
- ⏭️ AI Agent層: 該当ファイルなし（スキップ）
- ✅ DB層: レビュー完了
- ⏭️ Controller層: 該当ファイルなし（スキップ）
- ✅ Service層: レビュー完了
- ✅ その他: レビュー完了
```
...
2. 総評・まとめ・良い点の言及は不要
   - 「総評」「総合評価」「まとめ」「全体として」「技術的評価」「全体評価」「主な変更内容」などのセクションを作成しない
   - 「〜が良いです」「〜は適切です」などのポジティブな評価を記載しない
   - レビュー結果の形式（🔴修正必須、🟡修正推奨）は問題点の整理であり、総評ではないため使用する
   - レビュー完了時も指摘事項とレビュー状況のみを出力する
...
#### Nullability / Optionalityの厳格な審査

nullable/optionalの追加はデフォルトで修正必須として扱う。以下の条件を満たす場合のみ許容：

- 業務要件として「値なし」に意味がある（例：退職日は在職中null）
- 外部システム連携で値が保証されない

層別の確認ポイント：

- DB層: nullableにする業務要件を確認。「とりあえずnullable」は不可
- DTO層: optional propertyには省略時のデフォルト動作を明示させる
- Service層: null checkを追加する前に、呼び出し元でnullを排除できないか確認

#### 二重定義・状態増加の厳格な審査

新しいカラム/プロパティの追加はデフォルトで修正必須として扱う。以下を確認し、正当化できない場合は削除を求める：

- 他のカラムやリレーションから計算・導出できないか
- 同じ情報が別テーブル/エンティティに既に存在しないか

許容される例外：

- パフォーマンス要件が計測値で示されているキャッシュ用冗長カラム

指摘すべきパターン：

- リレーション先から取得可能な値を別カラムで保持
- 同じEnumやインターフェースを複数ファイルで再定義
- 「便利だから」「将来使うかも」という理由での追加
