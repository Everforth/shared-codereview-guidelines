...
- コードレビューを以下の順序で実施してください。順番に確認、どの層で問題があるかを明らかにしてからレビューしてください。各層で指摘点を発見した場合は、その層の問題解決に集中し、次の以降のレビューをしないでください。どの層についてレビューしたかを掲記してください。
  1. DB層（Entity定義, 型定義 等）
     - データモデルの整合性, 制約・インデックスの適切性 等
  2. Service層
     - ビジネスロジックの正確性, トランザクション境界の適切性 等
  3. Controller層
     - エンドポイント設計, バリデーション・エラーハンドリング 等
  4. その他
     - テストコード, 設定ファイル 等
- 良い点の言及・総評は不要
- コードの可読性と保守性を重視する
- 既存のコードを幅広く確認し、既存の設計パターンやベストプラクティスに従う
- パフォーマンスとセキュリティを考慮する
