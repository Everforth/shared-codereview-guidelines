...
STEP 1で特定した層をレビューし、🔴修正必須・🟡修正推奨がない場合は、必ずSTEP 1に戻り次の層（DB→Controller→Service→その他の順）を確認してください。

【レビュー順序】必ず以下の順序で全層をレビューすること
DB層 → Controller層 → Service層 → その他

【完了条件】

- 🔴修正必須・🟡修正推奨がある場合：問題を指摘してレビュー終了、修正待ち
- 🔴修正必須・🟡修正推奨がない場合：全層完了まで継続
...
3. 層のレビュー完了後の処理
   ...
   - 🔴修正必須・🟡修正推奨がある場合は問題を指摘してレビュー終了、修正を待つ
