...
以下はAI Agent層ではなく、他の層でレビューすること：

- **Entity定義**（ChatMessage, RunStep等のスキーマ設計）→ DB層で扱う
- **APIエンドポイント定義**（Agent呼び出しのエンドポイント）→ Controller層で扱う
- **ビジネスロジック**（Tool内で呼び出されるService処理）→ Service層で扱う
