...
- TypeORMの慣習尊重: デフォルト値の明示やコンストラクタでの全プロパティ初期化など、TypeORMの一般的なパターンに反する指摘をしない
...
- nullable設計: 値がない状態に明確な意味がある場合のみnullableにする
- 冗長なオプション指定の禁止: TypeORMデコレータで`nullable: false`などのデフォルト値は明示しない
- 型設計: 既存型定義を再利用しているか
...
- nullと空文字の使い分け: nullと空文字を区別する明確なユースケースがない場合は、nullableではなく`default: ''`を使用する
- nullableカラムの型定義: nullableにした場合はoptional markもつける（例: `price?: number`）
