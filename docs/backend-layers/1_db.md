...
- **命名による責務の明確性**: ファイル内のすべてのテーブル名、カラム名、インデックス名、制約名を確認し、今回追加された名前が既存の類似した名前と責務の違いを判別できるかを確認する。判別できない場合は🔴修正必須として指摘する
...
- 外部キー定義: ManyToOneリレーションを定義する場合、対応する外部キーカラムも明示的に定義する
- Entityのコンストラクタ:
