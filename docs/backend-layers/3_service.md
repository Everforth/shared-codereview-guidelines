...
- 類似した名前の変数・メソッド・クラスがある場合、それぞれの責務の違いが命名から判別できるか
- 類似した名前のメソッド・クラスがある場合、どちらをデフォルトで使用すべきかが命名から明確にわかるか, 処理そのものが命名に反映されているか
- 2つの操作が「ほぼ同じだが、追加の挙動がある」場合、メソッドを分けるのではなく、オプションで追加の挙動を制御する
