...
- **メソッド名と責務の一致**: メソッド名から挙動が推測できること。類似機能を持つ複数メソッドは責務の違いが命名から明確に区別できること
- **設定値の明示化**: `undefined` や `null` が「デフォルト動作」「処理スキップ」など特別な意味を持たないこと。意図を表す明示的な値を使用し、設定漏れを型で検出できるようにする
