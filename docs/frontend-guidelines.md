...
STEP 1で特定した層をレビューし、🔴修正必須・🟡修正推奨がない場合は、必ずSTEP 1に戻り次の層（モデリング→プロトタイピング→表示・スタイリング→データ処理の順）を確認してください。

【レビュー順序】必ず以下の順序で全層をレビューすること
モデリング層 → プロトタイピング層 → 表示・スタイリング層 → データ処理層

【完了条件】

- 問題を発見した層がある場合：その層で停止
- 問題がない層のみの場合：全層完了まで継続
...
レビューを完了する際は、どの層まで確認済みかを以下の形式で明記してください:
...
3. 層のレビュー完了後の処理
   - 🔴修正必須・🟡修正推奨がない場合は次の層のレビューを自動実施する
   - 問題を発見した時点で停止し、修正を待つ
