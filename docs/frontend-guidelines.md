...
レビュー依頼の内容を確認し、以下のどちらに該当するか判定してください：

**A. 設計相談・方針検討**

以下のような場合は、ガイドライン準拠チェックではなく設計相談として対応：

- 設計パターンの比較（例：「〜と〜どちらが良いか」）
- アーキテクチャ選択の相談
- 実装方針への意見・フィードバックを求める依頼
- 「〜が気になる」「〜は問題か」などの質問

→ **STEP 1-3をスキップし、それぞれのメリットとデメリットを明確に示しながら質問に直接回答する**
→ ガイドラインファイルの参照は不要

**B. コードレビュー（準拠チェック）**

- 上記に該当しない場合（無言での呼び出し、「レビューしてください」など一般的なレビュー依頼）
- → 以下のSTEP 0の内容を実施し、STEP 1以降のプロセスに進む
