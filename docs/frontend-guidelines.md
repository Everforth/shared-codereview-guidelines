...
- APIレスポンス仕様でnullが返される（OpenAPI定義等で確認済み）
- 初期状態で値が存在しないことに業務上の意味がある（例：未選択状態）

指摘すべきパターン：

- 「nullになるかもしれない」という推測のみでoptionalにしている
- APIがnullableでも、受信直後にデフォルト値を設定できる場合
- undefinedとnullが混在している（どちらかに統一を求める）

#### 二重定義・状態増加の厳格な審査

新しいstate/ref/reactiveの追加はデフォルトで修正必須として扱う。以下を確認し、正当化できない場合は削除または統合を求める：

- 既存の状態からcomputedで導出できないか
- Storeに同等のデータが既に存在しないか

指摘すべきパターン：

- computedで計算可能な値をstateとして保持
- Storeのデータをコンポーネントのローカル状態にコピー
- 既存の型と同じ構造を新規定義（extend/Pick/Omitで再利用すべき）
